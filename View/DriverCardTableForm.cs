...
{
    public partial class DriverCardTableForm : Form
    {
        public DriverCardTableForm()
        {
            InitializeComponent();
        }

        private void DriverCardTableForm_Load(object sender, EventArgs e)
        {
            FillTable();
        }
        ...
        private void FillTable()
        {
            ...
            {
                var data = DriverCard.SelectList(server.Connection);
                lvTable.Items.Clear();
                ...
                foreach (var row in data.Tables[0].Rows.Cast<DataRow>())
                {
                    ...
                    lvi.SubItems.Add(row["Служ_номер"].ToString());
                    lvi.SubItems.Add(row["ФИО"].ToString());
                    lvi.SubItems.Add(row["Класс"].ToString());
                    lvi.SubItems.Add(row["Категория"].ToString());
                    lvi.SubItems.Add(row["ID_карты"].ToString());
                    lvi.SubItems.Add(row["Тариф_час"].ToString());
                }
            }
        }
        ...
        {
            var frm = new DriverCardDetailForm();
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (var server = new Server())
                    {
                        var id = DriverCard.AddItem(server.Connection, frm.Data);
                        FillTable();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (lvi != null)
            {
                try
                {
                    ...
                    using (var server = new Server())
                    {
                       var item = DriverCard.SelectItem(server.Connection, id);
                       frm.Build(item);
                       if (frm.ShowDialog() == DialogResult.OK)
                       {
                           DriverCard.ChangeItem(server.Connection, id, item);
                           FillTable();
                ...
                MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                if (lvi != null)
                {
                    try
                    {
                        var id = (int)lvi.Tag;
                        using (var server = new Server())
                            DriverCard.RemoveItem(server.Connection, id);
