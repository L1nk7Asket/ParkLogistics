...
{
    public partial class ServiceTableForm : Form
    {
        ...
        private void ServiceTableForm_Load(object sender, EventArgs e)
        {
            FillTable();
        }
        ...
        private void FillTable()
        {
            ...
            {
                var data = Service.SelectList(server.Connection);
                lvTable.Items.Clear();
        ...
        private void btnAppend_Click(object sender, EventArgs e)
        {
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (var server = new Server())
                    {
                        var id = Service.AddItem(server.Connection, frm.Data);
                        FillTable();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (lvi != null)
            {
                try
                {
                    ...
                    using (var server = new Server())
                    {
                       var item = Service.SelectItem(server.Connection, id);
                       frm.Build(item);
                       if (frm.ShowDialog() == DialogResult.OK)
                       {
                           Service.ChangeItem(server.Connection, id, item);
                           FillTable();
                ...
                MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                if (lvi != null)
                {
                    try
                    {
                        var id = (int)lvi.Tag;
                        using (var server = new Server())
                            Service.RemoveItem(server.Connection, id);
