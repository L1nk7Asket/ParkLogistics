...
{
    public partial class FuelTableForm : Form
    {
        public FuelTableForm()
        {
            InitializeComponent();
        }

        private void FuelTableForm_Load(object sender, EventArgs e)
        {
            FillTable();
        }

        /// <summary>
        /// Загрузка таблицы ГСМ
        /// </summary>
        private void FillTable()
        {
            using (var server = new Server())
            {
                var data = Fuel.SelectList(server.Connection);
                lvTable.Items.Clear();
                ...
                foreach (var row in data.Tables[0].Rows.Cast<DataRow>())
                {
                    ...
                    lvi.SubItems.Add(((float)row["Плотность"]).ToString("0.000"));
                    lvi.SubItems.Add(row["Ед_измер"].ToString());
                }
            }
        }
        ...
        private void btnAppend_Click(object sender, EventArgs e)
        {
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (var server = new Server())
                    {
                        var id = Fuel.AddItem(server.Connection, frm.Data);
                        FillTable();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (lvi != null)
            {
                try
                {
                    ...
                    using (var server = new Server())
                    {
                       var item = Fuel.SelectItem(server.Connection, id);
                       frm.Build(item);
                       if (frm.ShowDialog() == DialogResult.OK)
                       {
                           Fuel.ChangeItem(server.Connection, id, item);
                           FillTable();
                ...
                MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                if (lvi != null)
                {
                    try
                    {
                        var id = (int)lvi.Tag;
                        using (var server = new Server())
                            Fuel.RemoveItem(server.Connection, id);
