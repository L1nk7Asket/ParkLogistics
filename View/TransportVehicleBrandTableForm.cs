...
{
    public partial class TransportVehicleBrandTableForm : Form
    {
        public TransportVehicleBrandTableForm()
        {
            InitializeComponent();
        }

        private void TransportVehicleBrandTableForm_Load(object sender, EventArgs e)
        {
            FillTable();
        }

        /// <summary>
        /// Заполнение таблицы марок
        /// </summary>
        private void FillTable()
        {
            using (var server = new Server())
            {
                var data = TransportVehicleBrand.SelectList(server.Connection);
                lvTable.Items.Clear();
                ...
                foreach (var row in data.Tables[0].Rows.Cast<DataRow>())
                {
                    ...
                    lvi.SubItems.Add(TransportVehicleType.SelectItem(server.Connection, (int)row["ТипТС"]).Name);
        ...
        {
            var frm = new TransportVehicleBrandDetailForm();
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (var server = new Server())
                    {
                        var id = TransportVehicleBrand.AddItem(server.Connection, frm.Data);
                        FillTable();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (lvi != null)
            {
                try
                {
                    ...
                    using (var server = new Server())
                    {
                       var item = TransportVehicleBrand.SelectItem(server.Connection, id);
                       frm.Build(item);
                       if (frm.ShowDialog() == DialogResult.OK)
                       {
                           TransportVehicleBrand.ChangeItem(server.Connection, id, item);
                           FillTable();
                ...
                MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                if (lvi != null)
                {
                    try
                    {
                        var id = (int)lvi.Tag;
                        using (var server = new Server())
                            TransportVehicleBrand.RemoveItem(server.Connection, id);
