...
{
    public partial class DivisionTableForm : Form
    {
        public DivisionTableForm()
        {
            InitializeComponent();
        }

        private void DivisionTableForm_Load(object sender, EventArgs e)
        {
            FillTable();
        }

        /// <summary>
        /// Загрузка таблицы подразделений
        /// </summary>
        private void FillTable()
        {
            using (var server = new Server())
            {
                var data = Division.SelectList(server.Connection);
                lvTable.Items.Clear();
        ...
        {
            var frm = new DivisionDetailForm();
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (var server = new Server())
                    {
                        var id = Division.AddItem(server.Connection, frm.Data);
                        FillTable();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (lvi != null)
            {
                try
                {
                    ...
                    using (var server = new Server())
                    {
                       var item = Division.SelectItem(server.Connection, id);
                       frm.Build(item);
                       if (frm.ShowDialog() == DialogResult.OK)
                       {
                           Division.ChangeItem(server.Connection, id, item);
                           FillTable();
                ...
                MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                if (lvi != null)
                {
                    try
                    {
                        var id = (int)lvi.Tag;
                        using (var server = new Server())
                            Division.RemoveItem(server.Connection, id);
        ...
        /// <summary>
        /// Двойной щелчок вызывает редактирования записи
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
