...
{
    public partial class RateTableForm : Form
    {
        public RateTableForm()
        {
            InitializeComponent();
        }

        private void RateTableForm_Load(object sender, EventArgs e)
        {
            FillTable();
        }
        ...
        private void FillTable()
        {
            ...
            {
                var data = Rate.SelectList(server.Connection);
                lvTable.Items.Clear();
                ...
                foreach (var row in data.Tables[0].Rows.Cast<DataRow>())
                {
                    ...
                    lvi.SubItems.Add(row["Ед_ставки"].ToString());
                    lvi.SubItems.Add(row["Объем"].ToString());
                }
            }
        }
        ...
        private void btnAppend_Click(object sender, EventArgs e)
        {
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    using (var server = new Server())
                    {
                        var id = Rate.AddItem(server.Connection, frm.Data);
                        FillTable();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (lvi != null)
            {
                try
                {
                    ...
                    using (var server = new Server())
                    {
                       var item = Rate.SelectItem(server.Connection, id);
                       frm.Build(item);
                       if (frm.ShowDialog() == DialogResult.OK)
                       {
                           Rate.ChangeItem(server.Connection, id, item);
                           FillTable();
                ...
                MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                if (lvi != null)
                {
                    try
                    {
                        var id = (int)lvi.Tag;
                        using (var server = new Server())
                            Rate.RemoveItem(server.Connection, id);
